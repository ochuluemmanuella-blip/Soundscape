...
func main() {
	connstr := "postgres://:sercret@localhost:5432/mydb?sslmode=disable"
	...
	err = db.Ping()
	if err != nil {
		log.Fatalf("cannot connect t0 Postgres: %v", err)
	}
	...
	insertQuery := "INSERT INTO users (email) VALUES ($1) ON CONFLICT DO NOTHING"
	_, err := db.Exec(insertQuery, "test@example.com")
	if err  != nil {
	...
	selectQuery := "SELECT id, email FROM users WERE email = $1"
